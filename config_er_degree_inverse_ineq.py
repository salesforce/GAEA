...
config["er-p"] = 0.1  #iterations in the graph editing loop
config["immunized nodes"] = {"lambda:degree": {"ascending":True, "fn": graph_inc.degree_scaled, "fn params":{}, "count": 3}}                                              #immunized node selection (e.g. 5 random)
config["graph fn"] = graph_inc.get_graph                                               #graph generator function
config["graph params"] = {"model": "er", "im": config["immunized nodes"],
                          "params": {"N":config["N"], "p":config["er-p"],
